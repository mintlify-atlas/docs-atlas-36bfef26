...
```bash
# Rename workspace
cmux workspace.rename "Frontend Dev"

# Set custom color (hex format)
cmux workspace.set-color "#C0392B"
```
...
Pin important workspaces to keep them at the top of the sidebar:

```bash
cmux workspace.pin
cmux workspace.unpin
```

Pinned workspaces stay in position even when new notifications arrive in other workspaces.
...
- Use the CLI: `cmux workspace.close`
